...
scoreHeight=62
scoreWidth=0
scoreFontFace="Comic-Sans-MS"
scoreFontColor=white
...
comboHeight=71
comboWidth=0
comboFontFace="Comic-Sans-MS"
comboFontColor=white
comboOutlineColor=#777777
comboOutlineWidth=1
comboMarginTop=0
comboMarginBot=-5
comboXText="x"
...
if $circleGenerate; then
    ...
    for i in {0..9}; do
        $runConvert $circleParams -fill "#$(openssl rand -hex 3)" label:"$i" "fontgen-temp/default-$i@2x.png";
    done;
    echo "Done!";
fi
...
if $scoreGenerate; then
    ...
    for i in {0..9} "," "." "%"; do
        $runConvert $scoreParams -fill "#$(openssl rand -hex 3)" label:"$i" "fontgen-temp/score-$i@2x.png";
    done;
...
if $comboGenerate; then
    ...
    for i in {0..9} "," "." "%"; do
        $runConvert $comboParams -fill "#$(openssl rand -hex 3)" label:"$i" "fontgen-temp/combo-$i@2x.png";
    done;
